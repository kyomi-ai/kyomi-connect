...
impl DatasourceProvider for DatabricksProvider {
    ...
    async fn execute_query_stream(
        ...
        tracing::debug!(
            sql = %prepared.sql.chars().take(200).collect::<String>(),
            "Streaming Databricks query"
        );

        // Submit the statement and wait for the initial response.
        // This happens before spawning so errors propagate directly.
        let initial_result = self.submit_statement(&prepared.sql).await?;
        ...
        let statement_id = initial_result
            .get("statement_id")
            .and_then(|id| id.as_str())
            .unwrap_or("")
            .to_string();

        // Clone fields needed by the spawned task for fetching subsequent chunks.
        ...
        tokio::spawn(async move {
            ...
            // Extract and emit the first chunk from the initial response
            let first_chunk_rows: Vec<Vec<Value>> = initial_result
                .get("result")
                .and_then(|r| r.get("data_array"))
                .and_then(|d| d.as_array())
                .map(|data| {
            ...
            if !first_chunk_rows.is_empty() {
                total_rows_returned += first_chunk_rows.len() as u64;
                if tx
            ...
            // Follow next_chunk_index links for subsequent pages
            let mut next_chunk = initial_result
                .get("result")
                .and_then(|r| r.get("next_chunk_index"))
                .and_then(|idx| idx.as_u64());

            while let Some(api_chunk_index) = next_chunk {
                let url = format!(
                    "https://{}/api/2.0/sql/statements/{}/result/chunks/{api_chunk_index}",
                    ...
                    Err(_) => {
                        let _ = tx
                            .send(Err(Error::Internal(format!(
                                "Databricks chunk {api_chunk_index} fetch timed out after {}s",
                                crate::DATASOURCE_TIMEOUT_QUERY.as_secs()
                            ))))
                            .await;
                        return;
                ...
                // Extract rows from this chunk and yield them
                let chunk_rows: Vec<Vec<Value>> = chunk_body
                    .get("data_array")
                    .and_then(|d| d.as_array())
                    .map(|data| {
                        data.iter()
