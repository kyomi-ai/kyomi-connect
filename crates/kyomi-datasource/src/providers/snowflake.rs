...
impl SnowflakeProvider {
    ...
    /// Fetch a single result partition from the Snowflake SQL API.
    ///
    /// After the initial `submit_statement` call returns the first partition
    /// (partition 0) inline, subsequent partitions are fetched individually
    /// via `GET /api/v2/statements/{handle}?partition={index}`.
    ///
    /// Returns the raw JSON response body for the partition.
    async fn fetch_partition(
        ...
        let url = format!(
            "https://{account}.snowflakecomputing.com/api/v2/statements/{handle}?partition={partition_index}"
        );
        ...
        if status.is_client_error() || status.is_server_error() {
            ...
            return Err(Error::Internal(format!(
                "Snowflake partition {partition_index} error: {msg}"
            )));
        }
...
impl DatasourceProvider for SnowflakeProvider {
    ...
    async fn execute_query_stream(
        ...
        tracing::debug!(
            sql = %paginated_sql.chars().take(200).collect::<String>(),
            "Streaming Snowflake query"
        );

        // Submit the query and wait for the first result. Errors
        // (syntax, permissions, polling timeout) propagate directly
        // before we spawn the background task.
        let initial_result = self.submit_statement(&paginated_sql).await?;

        // Extract column metadata from resultSetMetaData.rowType.
        let columns: Vec<ColumnInfo> = initial_result
            .get("resultSetMetaData")
            .and_then(|meta| meta.get("rowType"))
            .and_then(|rt| rt.as_array())
            .map(|row_types| {
        ...
        // Extract the statement handle (needed to fetch subsequent partitions).
        let statement_handle = initial_result
            .get("statementHandle")
            .and_then(|h| h.as_str())
            .unwrap_or("")
            .to_string();

        // Determine the total number of partitions from partitionInfo.
        let partition_count = initial_result
            .get("resultSetMetaData")
            .and_then(|meta| meta.get("partitionInfo"))
            .and_then(|pi| pi.as_array())
            .map(|arr| arr.len() as u64)
            .unwrap_or_else(|| {
                tracing::warn!("Snowflake response missing partitionInfo, assuming 1 partition");
                1
            });
        ...
        tokio::spawn(async move {
            ...
            // Extract and emit the first partition's rows from the initial response.
            let first_rows: Vec<Vec<Value>> = initial_result
                .get("data")
                .and_then(|d| d.as_array())
                .map(|data| {
                    data.iter()
            ...
            // Fetch subsequent partitions (partition 1, 2, ...).
            for partition_idx in 1..partition_count {
                let partition_body = match SnowflakeProvider::fetch_partition(
                    &client,
                    &account,
                    &token,
                    auth_type,
                    ...
                    Err(e) => {
                        let _ = tx
                            .send(Err(Error::Internal(format!(
                                "Failed to fetch Snowflake partition {partition_idx}: {e}"
                            ))))
                            .await;
                        return;
                    }
                ...
                let partition_rows: Vec<Vec<Value>> = partition_body
                    .get("data")
                    .and_then(|d| d.as_array())
                    .map(|data| {
                        data.iter()
                ...
                if !partition_rows.is_empty() {
                    total_rows_returned += partition_rows.len() as u64;
                    if tx
                        .send(Ok(QueryStreamEvent::Chunk {
                            rows: partition_rows,
                            chunk_index,
